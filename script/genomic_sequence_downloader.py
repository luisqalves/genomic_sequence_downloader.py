...
def check_target_gene_annotation_status():
    id = (
        os.popen(
            ...
            + "[GENE] AND "
            + current_species
            + '[Organism]" -sort "Relevance" | esummary | xtract -pattern DocumentSummary -element Id'
        )
        .read()
        .split("\n")[0]
        .replace("\n", "")
...
def download_via_existent_gene_annotation(id):
    species_common_name = (
        os.popen(
            "esummary -db gene -id "
            + id
            + " | xtract -pattern DocumentSummary -element CommonName"
        )
        .read()
        .replace("\n", "")
    )
    data = (
        os.popen(
            "esummary -db gene -id "
            + id
            + " | xtract -pattern DocumentSummary -if LocationHist -block LocationHistType -element ChrAccVer ChrStart ChrStop AssemblyAccVer"
        )
        .read()
        .split("\t")
    )
    if data == [""]:
        data = (
            os.popen(
                "esummary -db gene -id "
                + id
                + " | xtract -pattern DocumentSummary -if GenomicInfo -block GenomicInfoType -element ChrAccVer ChrStart ChrStop AssemblyAccVer"
            )
            .read()
            .split("\t")
        )
    ...
    print(
        args.target_gene_name
        + " annotation has been found.\nDownloading the genomic sequence portion underlying "
        + args.target_gene_name
        + " annotation..."
    )
    genomic_ID = data[0]
...
def download_via_synteny_conservation():
    print(
        current_species
        + " genome does not present annotation for the "
        + args.target_gene_name
        + " gene.\nDownloading through the synteny conservation-based method..."
    )
    upstream_flanking_genes = [
        args.first_upstream_gene_name,
        args.second_upstream_gene_name,
        args.third_upstream_gene_name,
    ]
    while len(upstream_flanking_genes) > 0:
        ...
        upstream_gene_data = (
            os.popen(
                ...
                + "[GENE] AND "
                + current_species
                + '[Organism]" -sort "Relevance" | esummary | xtract -pattern DocumentSummary -element Id Name'
            )
            .read()
            .split("\n")[0]
            .split("\t")
    ...
    downstream_flanking_genes = [
        args.first_downstream_gene_name,
        args.second_downstream_gene_name,
        args.third_downstream_gene_name,
    ]
    while len(downstream_flanking_genes) > 0:
        downstream_gene_name = downstream_flanking_genes[0]
        downstream_gene_data = (
            os.popen(
                'esearch -db gene -query "'
    ...
    downstream_gene_coordinates = (
        os.popen(
            "esummary -db gene -id "
            + downstream_gene_id
            + " | xtract -pattern DocumentSummary -if LocationHist -block LocationHistType -element ChrAccVer ChrStart ChrStop"
        )
        .read()
        .split("\t")
    )
    if downstream_gene_coordinates == [""]:
        downstream_gene_coordinates = (
            os.popen(
                "esummary -db gene -id "
                + downstream_gene_id
                + " | xtract -pattern DocumentSummary -if GenomicInfo -block GenomicInfoType -element ChrAccVer ChrStart ChrStop"
            )
            .read()
            .split("\t")
        )
    ...
    if downstream_sequence_id == upstream_sequence_id:
        assembly_ID = (
            os.popen(
                "esummary -db gene -id "
                + downstream_gene_id
                + " | xtract -pattern DocumentSummary -if LocationHist -block LocationHistType -element AssemblyAccVer"
            )
            .read()
            .replace("\n", "")
            .split("\t")[0]
        ...
        if assembly_ID == [""]:
            assembly_ID = (
                os.popen(
                    "esummary -db gene -id "
                    + downstream_gene_id
                    + " | xtract -pattern DocumentSummary -if GenomicInfo -block GenomicInfoType -element AssemblyAccVer"
                )
                .read()
                .replace("\n", "")
                .split("\t")[0]
